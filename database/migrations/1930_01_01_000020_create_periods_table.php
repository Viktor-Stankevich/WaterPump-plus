...
class CreatePeriodsTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('periods', function (Blueprint $table) {
            $table->id();
            $table->timestampTz('begin_date')->unique(); // Начало периода (1 число месяца 00:00:00)
            $table->timestampTz('end_date')->unique(); //   // Окончание периода (последнее число месяца 23:59:59)
        });
